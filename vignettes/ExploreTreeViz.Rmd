...
We create a `SingleCellExperiment` object, from one of the datasets provided in the `scRNAseq` package. We also calculate the dimensionality reductions; UMAP, TSNE and PCA from the functions provided in `scater` package and add it to the `SingleCellExperiment` object. 
...
We use the `createFromSCE` function to create a `TreeViz` object from `SingleCellExperiment` object. Here, the pipeline works in two ways:

1. If no cluster information is provided in the `colData` of the object, we create clusters at different resolutions using the `WalkTrap` algorithm by calling an internal function `generate_walktrap_hierarchy` on the `SingleCellExperiment` object. And use that cluster information for visualization
...
In both cases the user needs to privide the name of dimensionality reductions present in the object as a paramter in vector format
