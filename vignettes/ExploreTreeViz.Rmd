...
Start the App from the `treeViz` object we created. This should add a `facetZoom` to navigate the hierarchy, a heatmap of the top 100 most variable genes from the dataset and a scatter plot with TSNE showing various clusters. 
...
app <- startTreeviz()
treeViz <- find_top_variable_genes(treeViz, 100)

icicle_plot <- app$plot(treeViz, datasource_name = "SCRNA", tree = "col")
mes <- app$get_ms_object(chart_id_or_object = icicle_plot)

ms_list <- icicle_plot$get_measurements()
...
# add TSNE
app$chart_mgr$revisualize(chart_type = "PCAScatterPlot", chart= icicle_plot)
```
