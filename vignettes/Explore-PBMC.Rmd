...
To extract the selections, we provide a convenience method that returns
a `SingleCellExperiment` object for the provided dataset. The ColData of this object
will contain the cluster labels `treeviz_clusters` (column)
...
Now that we have clusters across resolutions, we can find marker genes for these clusters

```{r, eval=FALSE}
library(scater)
sce <- logNormCounts(sce)
markers <- findMarkers(sce, groups=sce$treeviz_clusters)
```

`markers` contains the results of the tests sorted by `p.value`.
