...
`TreeSummarizedExperiment` provides `aggregateTree` method, to aggregate counts (rows or columns) to a given tree selection. The result is a `TreeSummarizedExperiment` object. In a similar way, one can perform these operations on single cell datasets where colData has a hierarchy.
